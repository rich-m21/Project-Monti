...
namespace XDPaint.Core.PaintObject
{
    public sealed class SkinnedMeshRendererPaint : BasePaintObject
    {
        ...
        protected override bool IsInBounds(Vector3 position, Triangle triangle)
        {
            if (triangle == null)
                return false;
        ...
        protected override void CalculatePaintPosition(int fingerId, Vector3 position, Vector2? uv = null, bool usePostPaint = true, Triangle hitTriangle = null)
        {
            var paintObjectData = PaintObjectData[fingerId];
            paintObjectData.InBounds = IsInBounds(position, hitTriangle);
